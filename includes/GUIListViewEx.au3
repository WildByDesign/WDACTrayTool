...
Global $aGLVEx_Data[1][28] = [[0, 0, -1, "", -1, -1, -1, -1, _WinAPI_GetSystemMetrics(2), False, _
...
; [0][25] = Prog selection flag [n][25] - Header data array
; [0][26] = Checkbox flag       [n][26] - Edit field colour
;                               [n][27] - Column Order
...
Func _GUIListViewEx_Init($hLV, $aArray = "", $iStart = 0, $iColour = 0, $fImage = False, $iAdded = 0)
	...
	; Clear programatic selection flag
	$aGLVEx_Data[0][25] = -1
	...
	If BitAND($iAdded, 16) Then
		...
		; Fill array
		For $i = 0 To $iCols - 1
			; Read in current header title
			$aRet = _GUICtrlListView_GetColumn($hLV, $i)
			$aHdrData[0][$i] = $aRet[5]
			$aHdrData[1][$i] = ";"
			$aHdrData[2][$i] = Default
			$aHdrData[3][$i] = 0
		Next
	EndIf
	; Store array
	...
	$aRet = _GUICtrlListView_GetColumnOrderArray($aGLVEx_Data[$iLV_Index][0])
	$aGLVEx_Data[$iLV_Index][27] = _ArrayToString($aRet)
...
Func _GUIListViewEx_LoadListView($iLV_Index, $sFileName, $iDims = 2)
	...
	For $i = 0 To UBound($aHeader) - 1
		...
		$aHdrData[1][$i] = (($aHeader[$i][2]) ? ($aHeader[$i][2]) : (";"))
		$aHdrData[2][$i] = $aHeader[$i][1]
		$aHdrData[3][$i] = (($aHeader[$i][3]) ? ($aHeader[$i][3]) : (0))
	Next
	; Store array
	$aGLVEx_Data[$iLV_Index][25] = $aHdrData
...
Func _GUIListViewEx_InsertSpec($iLV_Index, $iRow, $vData, $fMultiRow = False, $fRetainWidth = False)
	...
	Local $vInsert, $iMax_Row, $aTempColInsert
	...
	; Check if 1D data array and if so transpose for correct colour insert
	If UBound($vInsert,0) = 1 And $fMultiRow = False Then
		_ArrayTranspose($vInsert)
	EndIf
	; Create colour array and set to default
	$aTempColInsert = $vInsert
	For $i = 0 To UBound($aTempColInsert, 1) - 1
		For $j = 0 To UBound($aTempColInsert, 2) - 1
			$aTempColInsert[$i][$j] = ";"
		Next
	Next
	...
	If $iRow = -1 Then
		...
		__GUIListViewEx_Array_Add($aGLVEx_SrcColArray, $aTempColInsert, $fMultiRow)
	Else
		...
		__GUIListViewEx_Array_Insert($aGLVEx_SrcColArray, $iRow + 1, $aTempColInsert, $fMultiRow)
	EndIf
...
;                                                + 4 - Edit filled with current value
;                                                + 8 - Edit all text selected
...
Func _GUIListViewEx_LoadHdrData($iLV_Index, $aHdrData)
	...
	; Check valid index
	If $iLV_Index < 0 Or $iLV_Index > $aGLVEx_Data[0][0] Then Return SetError(1, 0, 0)
	; Check header colour enabled
	If Not $aGLVEx_Data[$iLV_Index][24] Then
		Return SetError(3, 0, 0)
	EndIf
	; Check array
...
Func _GUIListViewEx_SelectItem($iRow, $iCol = -1)
	...
	Local $iStyle, $iOldRow = -1

	; Get active LV handle
	Local $hLVHandle = $aGLVEx_Data[$aGLVEx_Data[0][1]][0]
	Local $cLV_CID = $aGLVEx_Data[$aGLVEx_Data[0][1]][1]

	If $iRow = -1 Then ; Deselect all
		; Check for single selection style
		$iStyle = _WinAPI_GetWindowLong($hLVHandle, $GWL_STYLE)
		If BitAND($iStyle, $LVS_SINGLESEL) Then
			; Now need to check type of single selection
			If $aGLVEx_Data[$aGLVEx_Data[0][1]][22] Then ; Single cell selection
				; Set invalid row/col data in UDF arrays to clear all
				$iOldRow = $aGLVEx_Data[$aGLVEx_Data[0][1]][21]
				$aGLVEx_Data[0][2] = -1
				$aGLVEx_Data[0][17] = -1
				$aGLVEx_Data[0][18] = -1
				$aGLVEx_Data[0][25] = -1
				$aGLVEx_Data[$aGLVEx_Data[0][1]][20] = -1
				$aGLVEx_Data[$aGLVEx_Data[0][1]][21] = -1
			Else ; Standard single row selection
				Local $sSel = _GUICtrlListView_GetSelectedIndices($hLVHandle)
				If $sSel <> "" Then
					_GUICtrlListView_SetItemSelected($hLVHandle, $sSel, False)
					Return 1
				EndIf
			EndIf
		Else
			; Multiple selection so can use this function
	...
	Else
		...
		; Remove all current highlighting if multisel ListView
		_GUICtrlListView_SetItemSelected($hLVHandle, -1, False)
		...
		$aGLVEx_Data[0][25] = (($iCol = -1) ? (0) : ($iCol)) ; Prog sel flag
	...
	; Redraw ListView
	__GUIListViewEx_HighLight($hLVHandle, $cLV_CID, $iRow, $iOldRow)
...
;                                          Index number of "drag" ListView
;                                          Index number of "drop" ListView
;                                          Row number "from"
;                                          Row number "into"
;                                5 - Column resize event
;                                      Returns column index
;                                6 - Column reorder event
;                                      Returns original and new column order in string format: Number of columns|first|second|..|nth
;                                      Original and new strings separated by - delimiter
;                                7-8 (Not used at present)
...
Func _GUIListViewEx_EventMonitor($iEditMode = 0, $iDelta_X = 0, $iDelta_Y = 0)
	...
	; Check if checkbox actioned flag is set
	If $aGLVEx_Data[0][26] <> -1 Then
		; Clear existing selection and select row passed in flag
		_GUIListViewEx_SelectItem(-1)
		_GUIListViewEx_SelectItem($aGLVEx_Data[0][26])
		; Clear flag
		$aGLVEx_Data[0][26] = -1
	EndIf

	; Check for column drag
	If $fGLVEx_ColDrag Then
		$aRet = _GUICtrlListView_GetColumnOrderArray($aGLVEx_Data[$aGLVEx_Data[0][1]][0])
		Local $sOrder = _ArrayToString($aRet)
		Local $sOldOrder = $aGLVEx_Data[$aGLVEx_Data[0][1]][27]
		$aGLVEx_Data[$aGLVEx_Data[0][1]][27] = $sOrder
		$fGLVEx_ColDrag = False
		Return SetError(0, 6, $sOldOrder & "-" & $sOrder)
	EndIf
...
;                               - False   = Unregister WM_NOTIFY message
;                               - Default = Do nothing
;                  $fMOUSEMOVE  - True    = Register WM_MOUSEMOVE message
;                               - False   = Unregister $fMOUSEMOVE message
;                               - Default = Do nothing
;                  $fLBUTTONUP  - True    = Register WM_LBUTTONUP message
;                               - False   = Unregister $fLBUTTONUP message
;                               - Default = Do nothing
;                  $fSYSCOMMAND - True    = Register WM_SYSCOMAMND message
;                               - False   = Unregister $fSYSCOMMAND message
;                               - Default = Do nothing
; Requirement(s).: v3.3.10 +
; Return values .: None
; Author ........: Melba23
...
;                        MUST be final call in the handler and return value returned from other handler
...
Func _GUIListViewEx_MsgRegister($fNOTIFY = True, $fMOUSEMOVE = True, $fLBUTTONUP = True, $fSYSCOMMAND = True)
	...
	Switch $fNOTIFY
		Case Default
			; Do nothing
		Case True
			GUIRegisterMsg(0x004E, "_GUIListViewEx_WM_NOTIFY_Handler") ; $WM_NOTIFY
		Case False
	...
	Switch $fMOUSEMOVE
		Case Default
			; Do nothing
		Case True
			GUIRegisterMsg(0x0200, "_GUIListViewEx_WM_MOUSEMOVE_Handler") ; $WM_MOUSEMOVE
		Case False
			GUIRegisterMsg(0x0200, "")
	EndSwitch
	Switch $fLBUTTONUP
		Case Default
			; Do nothing
		Case True
			GUIRegisterMsg(0x0202, "_GUIListViewEx_WM_LBUTTONUP_Handler") ; $WM_LBUTTONUP
		Case False
			GUIRegisterMsg(0x0202, "")
	EndSwitch
	Switch $fSYSCOMMAND
		Case Default
			; Do nothing
		Case True
			GUIRegisterMsg(0x0112, "_GUIListViewEx_WM_SYSCOMMAND_Handler") ; $WM_SYSCOMMAND
		Case False
					...
					Case 196609 ; BitOR($CDDS_ITEMPREPAINT, $CDDS_SUBITEM)
						...
						If $iItem = $aGLVEx_Data[$iLV_Index][20] Then
							...
							If $aGLVEx_Data[$iLV_Index][22] Then
								If $aGLVEx_Data[0][25] <> -1 Then
									If $iSubItem = $aGLVEx_Data[0][25] Then
										$bSelColour = True
									EndIf
								Else
			...
			Switch $iCode
				...
				Case $LVN_COLUMNCLICK, -2 ; $NM_CLICK
					...
					If $iCode = $LVN_COLUMNCLICK Then
						...
						EndIf
					Else
						; It was a mouseclick, so set flag
						$fGLVEx_UserSelFlag = 1
						; Check if checkboxes used
						If $aGLVEx_Data[$iLV_Index][6] = 1 Then
							; Check if click was on checkbox
							Local $aTest = _GUICtrlListView_HitTest($hGLVEx_SrcHandle)
							If $aTest[0] <> -1 And $aTest[3] = False Then
								; Set checkbox flag to selected row
								$aGLVEx_Data[0][26] = $iRow
							EndIf
						EndIf
					EndIf
...
Func _GUIListViewEx_WM_LBUTTONUP_Handler($hWnd, $iMsg, $wParam, $lParam)
	...
	$sGLVEx_DragEvent = $iGLVEx_SrcIndex & ":" & $iGLVEx_TgtIndex & ":" & $iGLVEx_DraggedIndex & ":" & $iGLVEx_InsertIndex
...
Func __GUIListViewEx_HighLight($hLVHandle, $cLV_CID, $iIndexA, $iIndexB = -1)
	...
	; Redraw ListView
	__GUIListViewEx_RedrawWindow($aGLVEx_Data[0][1])
...
Func __GUIListViewEx_EditProcess($iLV_Index, $aLocation, $iDelta_X, $iDelta_Y, $iEditMode, $iForce = False)
	...
	Local $iEditType, $fEdit, $fCombo, $fRead_Only, $fAuto_Drop, $fAuto_Fill, $fAuto_Sel, $fDTP, $fClick_Move = False, $cUpDown, $hUpDown
	...
	While 1
		...
		Switch $iEditType
			...
			Case 2 ; Combo
				...
				$fAuto_Fill = (BitAND($aEditable[2][$aLocation[1]], 4) = 4)
				$fAuto_Sel = (BitAND($aEditable[2][$aLocation[1]], 8) = 8)
		...
		If $fAuto_Drop Then
			...
			EndSwitch
		EndIf
		; Check for combo additional parameters
		If $iEditType = 2 Then
			If $fAuto_Fill Then
				_SendMessage($hTemp_Edit, 0x000C, 0, $sItemOrgText, 0, "wparam", "wstr") ; $__EDITCONSTANT_WM_SETTEXT
			Else
				_SendMessage($hTemp_Edit, 0x000C, 0, "", 0, "wparam", "wstr")
			EndIf
			If $fAuto_Sel Then
				_SendMessage($hCombo, 0x142, 0, BitOR(BitShift(-1, -16), BitAND(0, 0xFFFF))) ; $CB_SETEDITSEL
			EndIf
		EndIf
		...
		Case 2
			For $i = 0 To UBound($aArray, $UBOUND_ROWS) - 1
				For $j = 0 To UBound($aArray, $UBOUND_COLUMNS) - 1
					; Cannot store user-defined function data
					If $j > 0 And $aArray[0][$j] = 9 Then
						$sRet &= $sDelim_Col
					Else
